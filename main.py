...
# -----------------Imports-----------------
    ...
    def update_parameters(start_x=23, start_y=580, row_spacing=13, padding=5, max_width=391):
        ...
        global button_list, parameter_map
    ...
    if not debug:
        ...
        The messages below would be your task. Just return the JSON format, and nothing else. Only the parameters I specified must be there in the JSON you return. Do not add or delete any keys.
        '''
        ...
        def ollama_chat():
            ...
            def save_and_update_param(my_text):
                global parameter_map
                data = json.loads(my_text)
                parameter_map = data
                update_parameters(start_x=23, start_y=580, row_spacing=13, padding=5, max_width=391)
            ...
            if previous != []:
                ...
                option_toplevel.config(bg="Grey30")
                ...
                tk.Label(option_toplevel, text="Response:", font=('Alte Haas Grotesk', 14, 'bold'), bg="Grey30", fg='White', justify='center').grid(column = 0, row = 0)
                ...
                save_button = customtkinter.CTkButton(option_toplevel, height=30, corner_radius=6, text='Save', font=('Alte Haas Grotesk', 15, 'bold'), text_color='White', width=100, command=lambda my_text=response['message']['content']: save_and_update_param(my_text))
                save_button.place(x=362, y=290, anchor=tk.SE)
                discard_button = customtkinter.CTkButton(option_toplevel, height=30, corner_radius=6, text='Discard', font=('Alte Haas Grotesk', 15, 'bold'), text_color='White', width=100, fg_color='IndianRed2', hover_color='IndianRed4', command = lambda: option_toplevel.destroy() if ext_funcs.temp_untop(option_toplevel, lambda: messagebox.askyesno("Discard changes?", "Are you sure you want to discard your changes?")) else None)
                discard_button.place(x=252, y=290, anchor=tk.SE)
    ...
    # ----------------------Simulation Results---------------------
    canvas.create_text(1766, 693, text='None yet.', font=('Alte Haas Grotesk', 12), anchor=tk.CENTER, fill='Grey50', tags='Sim_results')
    # -----------------------------End-----------------------------
